...
│   ├── src/
│   ├── migrations/   SQL schema migrations (custom runner, see src/db/migrate.ts)
...
| ORM/Query | node-postgres (`pg`) + hand-written SQL + a minimal custom migration runner | Explicit SQL keeps money/stock arithmetic auditable and avoids ORM-generated N+1/float traps. Migrations are plain, reviewable `.up.sql`/`.down.sql` file pairs. |
